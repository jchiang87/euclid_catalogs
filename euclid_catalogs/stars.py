...
__all__ = ["EuclidStarCollection", "EuclidStarObject"]
...
class EuclidStarObject(BaseObject):
    ...
    def __init__(self, ra, dec, sed_number, Av, ref_mag,  # noqa: N803
    ...
    def get_observer_sed_component(self, component, mjd=None):
        ...
        sed = self.collection.get_observed_sed(
            self.sed_number, self.Av, self.ref_mag)
        return sed
...
class EuclidStarCollection(ObjectCollection):
    ...
    def __init__(self, region, sky_catalog, sed_file, star_catalog_dir,
        ...
        self._object_class = EuclidStarObject
    ...
    def _apply_region_mask(self, region, star_cat_file):
        ...
        self._sed_number.extend(np.ma.array(sed_number, mask=mask)
                                .compressed())
    ...
    def get_observed_sed(self, sed_number, Av, ref_mag):  # noqa: N803
        lut = galsim.LookupTable(self.wl, self.flambda[sed_number],
                                 interpolant='linear')
    ...
    def __getitem__(self, index):
        if isinstance(index, int) or isinstance(index, np.int64):
            ra = self._ra[index]
            dec = self._dec[index]
            sed_number = self._sed_number[index]
            Av = self._Av[index]  # noqa: N806
            ref_mag = self._ref_mag[index]
            obj_id = self._id[index]
            return EuclidStarObject(ra, dec, sed_number, Av, ref_mag,
                                    obj_id, self, index)
